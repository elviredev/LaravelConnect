...
export default class Search {
    ...
    // Envoi d'une requête HTTP asynchrone à l'URL de Laravel
    // /search/{term}
    async sendRequest() {
    ...
    renderResultsHTML(posts) {
      ...
      <div class="list-group-item active"><strong>Résultats</strong> (${posts.length > 1 ? `${posts.length} articles trouvés` : "1 article trouvé"})</div>
      ${posts
                .map(post => {
        ...
        <span class="text-muted small">par ${post.user.username} le ${postDate.getDate()}/${postDate.getMonth() + 1}/${postDate.getFullYear()}</span>
      </a>`
                })
                .join("")}
    </div>`)
        } else {
            this.resultsArea.innerHTML = `<p class="alert alert-danger-pink text-center shadow-sm">Désolé, nous n'avons trouvé aucun résultat pour cette recherche.</p>`
        }
        this.hideLoaderIcon()
    ...
    <div class="search-overlay-top shadow-sm">
      <div class="container container--narrow">
        ...
        <input autocomplete="off" type="text" id="live-search-field" class="live-search-field" placeholder="Qu'est-ce qui vous intéresse?">
